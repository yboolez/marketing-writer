...
- **如果是 slides 或落地页** → 直接调 `frontend-slides` 渲染出 HTML，**不要在 CLI 里让用户审核中间结构/标题串**。做完后：
  ...
  **导出命令**（`~/.claude/skills/marketing-writer/scripts/export.sh`）：

  ```bash
  SH=~/.claude/skills/marketing-writer/scripts/export.sh

  bash "$SH" "<html>"              # PDF + 长图（默认）
  bash "$SH" "<html>" pdf          # PDF 只
  bash "$SH" "<html>" long         # 长图单张拼接
  bash "$SH" "<html>" moments      # 朋友圈 9 图（9:16，前 9 页）
  bash "$SH" "<html>" xhs          # 小红书多图（3:4，最多 18 页）
  bash "$SH" "<html>" all          # 一次出全部四种
  ```
