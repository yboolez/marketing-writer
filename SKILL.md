...
- **如果是 slides 或落地页** → 直接调 `frontend-slides` 渲染出 HTML，**不要在 CLI 里让用户审核中间结构/标题串**。做完后：
  ...
  **HTML 模板必须包含 export-mode 支持片段**（否则导出会错乱）：
  ...
  ```html
  <!-- 页面末尾 JS，识别 ?export=long 参数 + 演示模式下自动缩放防溢出 -->
  <script>
  // 导出模式开关
  (function () {
  ...
  // 演示 auto-fit（关键）：内容超出 100vh 时等比缩到刚好装下。
  // 没有这个，标题长/列表多的 slide 在小屏 / 短窗口上会被截断。
  // 导出模式跳过——那时要原尺寸或自由高度。
  (function autofit() {
    const mode = new URLSearchParams(location.search).get('export');
    if (mode) return;
    function fit(slide) {
      ...
      if (need > avail + 2) {
        ...
        c.style.transform = 'scale(' + (avail / need).toFixed(4) + ')';
      }
    }
  ...
  **关于 auto-fit 的重要说明**：
  - 这段 JS 在**演示 HTML** 时激活——测量每页内容真实高度，超出 viewport 就 `transform: scale()` 等比缩小。
  - **PDF 导出** 用 `@page` 锁定页面尺寸 + `-webkit-print-color-adjust: exact` 保真；auto-fit 跳过（否则 PDF 里每页会被缩小）。
  - **长图导出** 让内容自然展开；auto-fit 也跳过（否则内容还会被压缩）。
  - 这样一套兼顾：**演示不溢出、PDF 按原设计尺寸、长图自由高度**——三种场景都排版正确。
