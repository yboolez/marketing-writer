...
- **如果是 slides 或落地页** → 直接调 `frontend-slides` 渲染出 HTML，**不要在 CLI 里让用户审核中间结构/标题串**。做完后：
  ...
  **HTML 模板必须包含 export-mode 支持片段**（否则导出会错乱）：
  ...
  /* 长图不需要额外 CSS——export.sh 通过 ?slide=N 逐页截图再 PIL 拼接，
     每一页都按演示画布（100vh = 16:9）渲染，保证和 PDF / 演示完全一致 */
  ```

  ```html
  <!-- 页面末尾 JS：单页模式（长图逐页截图用） + 演示 auto-fit -->
  <script>
  // ?slide=N → 只显示第 N 页（供长图循环截图用）
  (function () {
  ...
  // Auto-fit：内容超出 slide 时等比缩到刚好装下。
  // 所有模式都跑（演示 / 单页截图 / 长图都需要一致外观）；
  // PDF 导出时 @media print 独立接管，auto-fit 不影响它。
  (function autofit() {
    function fit(slide) {
  ...
  **核心原则：固定画布（Fixed Canvas）**

  slides 是定义好长宽比的"页"（默认 16:9 widescreen，1920×1080）。**每个输出模式都尊重这个画布：**

  | 模式 | 每页怎么呈现 |
  |------|------------|
  | **演示 HTML** | 每 slide = 100vh，窗口变小时 auto-fit 自动缩放内容（不截断） |
  | **PDF** | `@page 13.333in × 7.5in` 锁定，每 slide 对应一页，print-color-adjust 保真 |
  | **长图** | `export.sh` 循环 `?slide=N` 按 1920×1080 逐页截屏 → PIL 拼接为单张 PNG |

  **字体可读性 ≠ 需要特殊 CSS**——长图默认输出 1920 宽（非 1080），手机上放大即可看清小字，不做压缩式缩放。如果要更小文件体积，`export.sh <html> long 1080` 指定窄版。

  **溢出是内容问题，不是渲染问题**：如果某页总是放不下，**拆成两页**才是正解。auto-fit 只是最后的安全网。
