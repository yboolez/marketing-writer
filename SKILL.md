...
description: |
  ...
  触发方式：/deck-writer、/营销文案、/说人话、「帮我写 pitch deck」「做个落地页」「写个营销 slides」「把这个写成对外宣传」「说人话」
  ...
  Trigger: /deck-writer, /说人话, "write a pitch deck", "make a landing page", "marketing slides", "说人话"
---
...
**你不是 PPT 生成器。你是销售话术编剧。**——决定说什么、什么顺序说、怎么开场，并自己渲染最终 HTML。
...
| 页面类型 | 处理 |
|---|---|
| **Hero** | 展示——3 秒让对方 get 到"这是什么" |
| **痛点 / 探索 / 案例** | 悬念——用「为什么」「怎么做到的」「你想不到的客户」铺垫，不抖答案 |
| **差异化 / 数据 / 结论** | 展示——直接给数字、对比、结论 |
| **CTA** | 展示——"现在联系我们"不需要悬念 |

反面模式：

> ❌ `李亚鹏 30 年干黄十几个项目，证明人脉不等于赚钱` ← 标题就把答案说完了
...
写不出好开头，90% 是内容本身的问题。Phase 1 硬性拦截按这 5 个维度检查：
...
| 形式 | 适合场景 | 交付 |
|------|---------|------|
| **Slides（16:9 多页 HTML）** | 融资 pitch、商务介绍、正式 BP | HTML，可导出 PDF / 长图 |
| **单页落地页** | 官网首页、对外宣传站 | HTML，主叙事给非专业用户，技术细节/pricing/FAQ 进导航 |
| **9:16 朋友圈 9 图** | 微信朋友圈 | HTML 按 9:16 设计，导出 9 张 PNG |
| **3:4 小红书多图** | 小红书笔记（≤18 张） | HTML 按 3:4 设计，导出多张 PNG |
| **长文 / 深度稿** | 公众号推送、官网博客、白皮书 | Markdown，每个小标题下 1-3 段完整段落 |
| **纯文案（短）** | 微信/邮件单条 | Markdown |

**长文模式的关键差别**：标题规则不变，但**正文展开 1-3 段完整段落**（每段 150-400 字），不是 bullet。

**单页落地页分层**：主流给非专业用户（Hero → 痛点 → 方案 → 案例 → CTA），导航栏放技术细节 / pricing / FAQ。
...
| **9:16 朋友圈** (1080×1920) | ≤ 80 | ≤ 3 | ≤ 5 | ≤ 4 (2×2) | 一句金句 + 1 视觉 |
| **3:4 小红书** (1080×1440) | ≤ 120 | ≤ 4 | ≤ 8 | ≤ 6 (2×3) | 标题 + 副标 + 1 视觉 |
| **长文 Markdown** | 段落 150-400 字 / 节 | 不限 | 不限 | — | — |

> 1080w 是行业共识（IG / 微信 / 小红书 / 微博），现代手机 DPR 2-3x，再低就糊。

**社交竖版字号（1080w 画布行业规范）**：

| 元素 | 推荐 px | 用途 |
|------|---------|------|
| Hero 主标题 | 95-130 | 封面金句 |
| 段落标题 | 70-90 | 各页 H1 |
| 正文 / 卡片标题 | 36-50 | 必须可读，最关键 |
| 标签 / meta | 28-34 | 角标、引语 |
| 最小可读 | 24 | 底线，再小就废 |

**重要陷阱**：CSS 用 `clamp(X-rem, Y-vw, Z-rem)` 时，rem 默认是 16px，max cap 给桌面写的（`5rem=80px`），在 1080w 画布上会把字卡得太小（理论需要 130px）。竖版模式必须**抬高 root font-size**（推荐 `documentElement.style.fontSize = '28px'`）让所有 rem-based clamp 同步放大；或者直接用 px 覆盖 `--title-xl / --body-size` 等变量。
...
- **如果是 slides 或落地页** → 按下文「HTML 视觉规范」自己渲染 HTML。**不要在 CLI 里让用户审核中间结构/标题串。** 做完后：
  1. **每个 deck 给它一个独立项目文件夹**，HTML 和将来所有导出都放在里面。所有 deck-writer 项目集中在 `~/Desktop/deck-writer/` 下：
     ```
     ~/Desktop/deck-writer/<项目名>/
     ├── <项目名>.html           ← 现在写
     ├── <项目名>.pdf            ← export 时生成
     ├── <项目名>.png            ← export 长图时生成
     ├── moments/01-09.png       ← export 朋友圈时生成
     └── xhs/01-14.png           ← export 小红书时生成
     ```
     export.sh 默认就把产物丢在 HTML 同目录——别把 HTML 直接写在 `~/Desktop/`，否则导出会污染桌面。如果 `~/Desktop/deck-writer/` 不存在则先 `mkdir -p`。
  2. 用 `open` 命令替用户在浏览器打开
  3. 告知存储路径
  4. **主动让用户勾选要哪些导出格式**——不要默认全跑：

     > 做完了，路径在 `~/Desktop/deck-writer/<项目名>/<项目名>.html`。要导出哪些？
     > - □ PDF（横版 PPT 风格，正式外发）
     > - □ 长图（移动端拼接单张 PNG，群发文章）
...
```bash
SH=~/.claude/skills/deck-writer/scripts/export.sh
...
| 触发条件 | 推荐 |
|---|---|
| 用户做的是小红书标题（不是 slides） | `/dbs-xhs-title` |
| 用户做的是短视频开头（不是 slides） | `/dbs-hook` |
| 用户的内容本身需要诊断 | `/dbs-content` |
| 通用 talk / 教学 / tech demo / PPTX 转换 | `frontend-slides` |
