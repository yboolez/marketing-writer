...
- **如果是 slides 或落地页** → 按下文「HTML 视觉规范」自己渲染 HTML。**不要在 CLI 里让用户审核中间结构/标题串。** 做完后：
  1. **每个 deck 给它一个独立项目文件夹**，HTML 和将来所有导出都放在里面。所有 marketing-writer 项目集中在 `~/Desktop/marketing-writer/` 下：
     ```
     ~/Desktop/marketing-writer/<项目名>/
     ├── <项目名>.html           ← 现在写
     ├── <项目名>.pdf            ← export 时生成
     ...
     ```
     export.sh 默认就把产物丢在 HTML 同目录——别把 HTML 直接写在 `~/Desktop/`，否则导出会污染桌面。如果 `~/Desktop/marketing-writer/` 不存在则先 `mkdir -p`。
  2. 用 `open` 命令替用户在浏览器打开
  3. 告知存储路径
  4. **主动让用户勾选要哪些导出格式**——不要默认全跑：

     > 做完了，路径在 `~/Desktop/marketing-writer/<项目名>/<项目名>.html`。要导出哪些？
     > - □ PDF（横版 PPT 风格，正式外发）
     > - □ 长图（移动端拼接单张 PNG，群发文章）
